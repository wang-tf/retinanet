...
def filter_detections(boxes, classification, other=[], nms=True, score_threshold=0.05, max_detections=300,
    ...
    for c in range(int(classification.shape[1])):
        ...
        score_indices = tf.where(keras.backend.greater(scores, score_threshold))
        score_indices = tf.cast(score_indices, 'int32')
        filtered_boxes = tf.gather_nd(boxes, score_indices)
        filtered_scores = tf.gather(scores, score_indices)[:, 0]
        filtered_other = [tf.gather_nd(o, score_indices) for o in other]
        ...
        if nms:
            ...
            filtered_boxes = tf.gather(filtered_boxes, nms_indices)
            filtered_scores = tf.gather(filtered_scores, nms_indices)
            filtered_other = [tf.gather(o, nms_indices) for o in filtered_other]
    ...
    boxes = tf.gather(boxes, top_indices)
    labels = tf.gather(labels, top_indices)
    other_ = [tf.gather(o, top_indices) for o in other_]
